...
function ListUserFiles($Thefile, $TheClass, $userID, $fl, $nx, $pr) {
	...
	if(isset($_POST['openscan' . $fl . ''])){
		$msg = 'From: <strong>' . $current_user->display_name . '</strong><br /><br />Open & Scan: <strong>' . pathinfo($Thefile, PATHINFO_FILENAME) . '</strong>';
		$headers = 'From: MailGapp User ' . $current_user->display_name . ' <contact@mailgapp.com>' . "\r\n";
		add_filter('wp_mail_content_type',create_function('', 'return "text/html";'));
		wp_mail( 'contact@mailgapp.com', 'Request to Open & Scan', $msg, $headers );
	}
	if(isset($_POST['destroy' . $fl . ''])){
		$msg = 'From: <strong>' . $current_user->display_name . '</strong><br /><br />Destroy: <strong>' . pathinfo($Thefile, PATHINFO_FILENAME) . '</strong>';
		$headers = 'From: MailGapp User ' . $current_user->display_name . ' <contact@mailgapp.com>' . "\r\n";
		add_filter('wp_mail_content_type',create_function('', 'return "text/html";'));
		wp_mail( 'contact@mailgapp.com', 'Request to Destroy Mail', $msg, $headers );
	}

	echo '<td class="' . $TheClass . '" width="60%" ><a data-uk-modal="{target:\'#' . $fl . '\'}" href="#' . $fl . '"><i class="uk-icon-file f-doc"></i>' . pathinfo($Thefile, PATHINFO_FILENAME) . '</a></td>';
	echo '<div id="' . $fl . '" class="uk-modal"><div class="uk-modal-dialog">';
	echo '<a class="uk-modal-close uk-close"></a><img src="' . $dnlLink . '"/><br />';

	echo '<a class="uk-button prev" data-uk-modal="{target:\'#' . $pr . '\'}" href="#' . $pr . '"><i class="uk-icon-chevron-left"></i></a>';
	echo '<a class="uk-button nex" data-uk-modal="{target:\'#' . $nx . '\'}" href="#' . $nx . '"><i class="uk-icon-chevron-right"></i></a>';

	echo '<div class="mdl-buttons uk-clearfix">';

	echo '<form method="POST" action="">';
	echo '<button type="submit" class="uk-button uk-button-success"><i class="uk-icon-share"></i> Open & Scan</button>';
	echo '<input type="hidden" name="openscan' . $fl . '" /></form>';

	echo '<form method="POST" action="">';
	echo '<button type="submit" class="uk-button uk-button-danger"><i class="uk-icon-trash"></i> Destroy Mail</button>';
	echo '<input type="hidden" name="destroy' . $fl . '" /></form>';

	echo '<a href="' . $DelLink . '" class="uk-float-right"><span class="uk-button uk-button-danger"><i class="uk-icon-remove"></i> Delete Image</span></a>';
	echo '</div></div></div>';
	...
	echo '<td class="' . $TheClass . '" align="right"><a data-uk-modal="{target:\'#' . $fl . '\'}" href="#' . $fl . '"><button class="f-dload"><i class="uk-icon-eye-open"></i></button></a>';
	...
	if (get_option('file_manger_allow_del') == 'yes') {
		...
		echo '  <a href="' . $DelLink . '"><button class="f-delete"><i class="uk-icon-remove"></i></button></a></td></tr>';
	} else {
		echo '</td></tr>';
	}//end if
}
...
function ListFilteredFiles($Thefile, $userID) {
	...
	echo '<tr><td  width="60%" ><input type="checkbox" name="change_cat' . $tp . '" value="addit" /> <input type="hidden" name="file' . $tp . '" value="' . $Thefile . '" ><input type="hidden" name="changecat_user' . $tp . '" value="' . $userNum . '"> ' . pathinfo($Thefile, PATHINFO_FILENAME) . '</td>';
	...
	echo '<td align="right"><a data-lightbox href="admin.php?page=manage-files-main&theDLfile=' . $userID . '/' . $Thefile . '"><i class="uk-icon-circle-arrow-down f-dload"><i/></a>';
	...
	if (get_option('file_manger_allow_del') == 'yes') {
		...
		echo '  <a href="admin.php?page=manage-files-main&deletefile=' . $userID . '/' . $files . '"><i class="uk-icon-minus-sign f-delete"><i/></a></td></tr>';
	} else {
		echo '</td></tr>';
	}//end if
	$tp++;
		...
		while (false !== ($files = readdir($Subhandle))) {
			...
			if ($files != ".") {
				if ($files != "..") {
					...
					echo '<td><input type="checkbox" name="change_cat' . $tp . '" value="addit" /> <input type="hidden" name="file' . $tp . '" value="' . $files . '" ><input type="hidden" name="changecat_user' . $tp . '" value="' . $userNum . '"> ' . pathinfo($files, PATHINFO_FILENAME) . '</td>';
					...
					echo '<td align="right"><a data-lightbox href="admin.php?page=manage-files-main&theDLfile=' . $userNum . '/' . $files . '">download</a> | <a href="admin.php?page=manage-files-main&deletefile=' . $userNum . '/' . $files . '">delete</a></td>';
...
function manage_files_user() {
	...
	?>
	<form class="uk-form" method="POST" action="admin.php?page=manage-files-user" >
	...
	<?php
	echo '<div class="wrap"><table class = "widefat" width="100%">';
	echo'<thead><th>';
	echo __('Your Files', 'userfiles');
	echo '</th><th>Date</th><th>Category</th><th></th></thead>';
				...
				if ($file != "..") {
					...
					if ($user_file_search) {
						...
						if ($searchMatch === 0 || $searchMatch > 0) {
							ListUserFiles($file, $rowClass, $current_user->ID, $i, $n, $p);
							$found = true;
						}
	...
	if ($currOpts_up == 'yes') {
		...
		<div class="wrap"><table class="widefat"><thead><th><h3><?php _e('Add Files', 'userfiles'); ?></h3></th></thead>
