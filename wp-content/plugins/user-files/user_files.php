...
/*
  ...
  Plugin URI: http://www.crodesign.com
  ...
  Author: CroDesign
  Version: 10.0.2
  Author URI: http://www.crodesign.com
 */
...
function files_settings_page() {
	...
	<?php
	if ($_GET['full_uninstall'] == true) {
		DectivateFileDir();
	}

	if (isset($_POST['removeit'])) {

		echo '<div id="message" class="updated highlight" style="padding:5px; width:auto;display: table;margin: 5px 0 20px 0;">';
	...
	if ($_POST['update']) {
		...
		echo '<div id="message" class="updated fade" style="padding:5px; width:auto;display: table;margin: 5px 0 20px 0;">' . __('Settings Saved', 'userfiles') . '</div>';
	}
	...
	<table class="form-table">
		...
		<form method="POST" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
			<td><input type="checkbox" name="file_manger_show_dash" value="yes" <?php if ($currOpts_dash == 'yes') {
		echo 'checked';
	} ?>> <?php _e('Show Dashboard Widget', 'userfiles'); ?><br></td>
			</tr>

			<tr>
				<td><input type="checkbox" name="file_manger_show_menu" value="yes" <?php if ($currOpts_menu == 'yes') {
		echo 'checked';
	} ?>> <?php _e('Show dashboard menu link', 'userfiles'); ?><br></td>
			</tr>
			<tr>
				<td><input type="checkbox" name="file_manger_allow_up" value="yes" <?php if ($currOpts_up == 'yes') {
		echo 'checked';
	} ?>> <?php _e('Allow users to add files', 'userfiles'); ?><br></td>
			</tr>
			<tr>
				<td><input type="checkbox" name="file_manger_allow_del" value="yes" <?php if ($currOpts_del == 'yes') {
		echo 'checked';
	} ?>> <?php _e('Allow users to delete files', 'userfiles'); ?><br /></td>
			</tr>
...
function manage_files_mainpg() {
	...
	if (isset($_GET['deletefile'])) {
		...
		if ($isitGone) {
			...
			echo '<div id="message" class="updated" style="padding:5px; width:auto;display: table;margin: 5px 0 20px 0;">';
			echo __('The file has been deleted', 'userfiles');
			echo '</div>';
		} else {
			echo '<div id="message" class="error" style="padding:5px; width:auto;display: table;margin: 5px 0 20px 0;">';
	...
	<form class="uk-form" method="POST" action="admin.php?page=manage-files-main" >
							...
							if ($file != ".") {
								if ($file != "..") {
									...
									if ($Subhandle = @opendir($upload_dir['basedir'] . '/file_uploads/' . $userNum)) {
											...
											if ($files != ".") {
												if ($files != "..") {
													...
													if ($FileCheck === 0 || $FileCheck > 0) {
														...
														echo '<td align="right"><a rel="download.png" href="admin.php?page=manage-files-main&theDLfile=' . $files . '"><i class="uk-icon-circle-arrow-down"></i></a> | <a href="admin.php?page=manage-files-main&deletefile=' . $files . '"><i class="uk-icon-minus-sign"><i/></a></td></tr>';
														$tp++;
													}
												}
											}
										...
										if ($endTable) {
											echo '</table></div><div align="left"><input type="submit" name="submit" value="' . __('Update', 'userfiles') . '" /></div><p>&nbsp;</p>';
										}
									}
								}
							}
...
######################
				...
				<h3>Manage Catagories</h3>
					...
					foreach ($getCats as $c) {
						...
						if ($currOpts_defcat != $c->category) {
							echo '<a href="admin.php?page=files-add-cats&defcat=' . $c->category . '">(Make Default)</a> | ';
						}
				...
				<form method="POST" action="admin.php?page=files-add-cats">
					<input type="text" id="addcat" value="<?php if ($err == 1) {
				echo $_POST['addcat'];
			} ?>" name="addcat" />
...
//$usermail = str_ireplace('%','',$usermail);
				...
				echo __('Select User for these Files', 'userfiles') . ': <br /><select name="curr_user" id="curr_user" style="width: 240px;">';
				foreach ($aUsersID as $iUserID) :
					...
					?>
						...
						<option value=<?php echo '"' . $iUserID . '">' . $user_info->first_name . ' ' . $user_info->last_name . ' / ' . $user_info->user_login; ?> </option>
					...
					<table>
						...
						<tr>
							<td>
								...
								<input name="uploadedfile" type="file" style="width: 230px;" /><br />
								<span style="font-size:90%;color:#ccc;float:right;">your upload limit is <?php echo $max_post; ?>M&nbsp;</span>
							</td>
						</tr>
						<tr>
							<td>
						...
						<tr>
							<td style="text-align: right;">
								<?php echo __('Notify User of Upload', 'userfiles'); ?>:<input type="checkbox" name="notify_user" value="checked" checked />
							</td>
						</tr>
						<tr>
							<td style="text-align: right;">
			...
			function manage_files_userpage() {
				...
				if (is_user_logged_in()) {
					...
					?>
						<form class="uk-form dark" method="POST" action="<?php echo $_SERVER['REQUEST_URI']; ?>" >

							Search: <input type="text" name="file_search" value="" class="f-search"/>
							...
							<select name="showcatsfilter" id="showcatsfilter">
								...
								<input class="uk-button" type="submit" name="sorted" value="<?php _e('Filter', 'userfiles'); ?>" />
					...
					if ($file != "..") {
						...
						if ($user_file_search) {
							...
							if ($searchMatch === 0 || $searchMatch > 0) {
								...
								ListUserFiles($file, $rowClass, $current_user->ID, $count, $next, $previous);
								$found = true;
							}
		...
		if ($currOpts_up == 'yes') {
			...
			?>
							...
							<div class="wrap"><table class="widefat"><thead><th><h3><?php _e('Add Files'); ?></h3></th></thead>
						...
						while (false !== ($file = readdir($handle))) {
							...
							if ($file != ".") {
								if ($file != "..") {
									...
									echo '<td width="10%" align="right"><a rel="download.png" href="index.php?theDLfile=' . $file . '"><i class="uk-icon-circle-arrow-down">download</i></td></tr></table> ';
								}
							}
							?>
...
####################################
									...
									function Icon_management() {
										...
										echo '<div class="wrap"><table class="widefat"><tr><thead><th>' . __('Icons', 'userfiles') . '</th><th></th></tr></thead>';
										foreach ($getIcons as $icon) {


											echo '<tr><td><i class="uk-icon-file f-doc"></i></td><td align="right"><a href="admin.php?page=files-add-icons&deleteicon=' . $icon->extension . '">(Delete)</a></td></tr>';
// echo '<tr><td><img src="'.$icon->image.'" width="20px" />   ' .$icon->extension.'</td><td align="right"><a href="admin.php?page=files-add-icons&deleteicon='.$icon->extension.'">(Delete)</a></td></tr>';
										...
										<div class="wrap"><table class="widefat">
												...
												<form enctype="multipart/form-data" action="admin.php?page=files-add-icons" method="POST">
													<tr><td> <input  name="uploadedicon" type="file" <?php if ($err1 == 2) {
										echo 'value="' . $_FILE['uploadedicon'] . '"';
									} elseif ($err == 1) {
										echo 'class="highlight"';
									} ?>/></td>

														<td> <?php echo __('Extension', 'userfiles'); ?> :<input type="text" name="extension" <?php if ($err2 == 2) {
										echo 'value="' . $_POST['extension'] . '"';
									} elseif ($err == 1) {
										echo 'class="highlight"';
									} ?> /></td></tr>
									...
									function FTP_Paths() {
										...
										foreach ($aUsersID as $iUserID) :
											...
											if (!$isFolder) {
												...
												echo '<td><form method="POST" action="admin.php?page=files-see-ftp"><input type="hidden" name="createFolder" value="' . $iUserID . '" /><input type="submit" value="';
												echo __('Create Folder', 'userfiles');
												echo'" /></form></td></tr>';
											} else {
									...
									function userfiles_credit() {
										...
										return 'Thank you for using MailGapp';
									}
